...
1. **No HTML file support:** Only markdown files are checked
2. **No anchor validation:** Internal anchors (#section) are not validated
3. **Bot protection:** Some sites block automated tools (Cloudflare, etc.)
4. **Rate limiting:** Checking many links to the same site may trigger rate limits
5. **No JavaScript links:** Dynamic links generated by JavaScript are not detected

## Further Reading

- [markdown-link-check on GitHub](https://github.com/tcort/markdown-link-check)
- [markdown-link-check on npm](https://www.npmjs.com/package/markdown-link-check)
