...
Implement a pre-commit Git hook that validates all links across the entire project repository before allowing commits. The hook will use the `lychee` link checker tool to perform comprehensive validation of both local file references and external URLs, blocking commits if any dead links are detected.

## Functional Requirements

### FR1: Link Detection and Scanning
- **FR1.1:** Scan all file types in the repository for links (markdown, HTML, source code, configuration files, etc.)
- **FR1.2:** Detect various link formats:
  ...
  - Plain URLs in comments and documentation
  - File path references in source code
...
- **FR3.5:** Show the file path and line number for each dead link

### FR4: Tool Installation and Configuration
- **FR4.1:** Install `lychee` as a development dependency or system requirement
- **FR4.2:** Configure `lychee` with appropriate settings:
  - Timeout values for HTTP requests
  - User-agent string
  - Redirect following behavior
  - Excluded URLs or patterns (if needed)
- **FR4.3:** Create a `.lycheeignore` or configuration file for exclusions

### FR5: Error Reporting
- **FR5.1:** Output clearly formatted error messages showing:
  - Total number of links checked
  - Number of dead links found
  - Each dead link with file path, line number, URL, and failure reason
...
- Cache successful URL checks to avoid redundant network requests (if supported by lychee)
...
- Configuration should be version-controlled (`.lycheeignore`, config files)
- Documentation should explain how to bypass the hook in emergencies (`--no-verify`)
- Clear instructions for installing lychee on different platforms
...
4. **AC4:** Error output clearly identifies each dead link with file path and line number
5. **AC5:** Both local file paths and external URLs are validated
6. **AC6:** HTTP redirects are followed correctly
7. **AC7:** Lychee configuration is documented and version-controlled
...
- Custom link checker implementation (using existing tool: lychee)
